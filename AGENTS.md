...
- **Project structure**: routes/ handles transport and protocol wiring, services/ owns business rules and use-case orchestration, dto/ defines request/response contracts plus validation/transformation structs, and repositories/ may be introduced later for external integration adapters; keep modules small and focused.
- **Layer rules**: routes/ may know transport concerns but must not contain business rules; services/ may orchestrate integration adapters and transform dto/ but must not depend on HTTP details; repositories/adapters may depend on external clients but must not return transport-layer responses; dto/ must not contain external access or business orchestration.
